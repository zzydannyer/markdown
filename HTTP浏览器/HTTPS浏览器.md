...
* 1.0版本是一种无连接、无状态的应用层协议
* 浏览器每次链接都需要与服务器j建立一个TCP连接
...
* 表示资源被正确的创建。
  * POST 用户名、密码正确创建了一个用户就可以返回 201

#### 202

* 请求是正确的，但是结果正在处理中
  * 这时候客户端可以通过`轮询`等机制继续请求

#### 203

* 请求的代理服务器修改了源服务器返回的 200 中的内容
  * 我们通过代理服务器向服务器 A 请求用户信息，服务器 A 正常响应，但代理服务器命中了缓存并返回了自己的缓存内容，这时候它返回 203 告诉我们这部分信息不一定是最新的，我们可以自行判断并处理
...
* 永久重定向，不允许改变请求方法，使用原有的地址请求方式来通过新地址获取资源
