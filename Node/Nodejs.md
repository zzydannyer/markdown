...
- Z：修订号，问题修正
  ...
  `^`：只会执行不更改最左边非零数字的更新。如 ^0.13.0，运行npm update可以更新到 0.13.1、0.13.2 等，但不能更新到 0.14.0 或更高版本。 ^1.13.0可以更新到 1.13.1、1.14.0 等，但不能更新到 2.0.0 或更高版本。
  `~`：如 〜0.13.0，运行 npm update 更新到补丁版本：即 0.13.1 可以，但 0.14.0 不可以。
  `>`：接受高于指定版本的任何版本。
  `>=`：接受等于或高于指定版本的任何版本。
  `<=`：接受等于或低于指定版本的任何版本。
  `<`：接受低于指定版本的任何版本。
